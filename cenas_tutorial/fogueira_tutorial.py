...
class CenaFogueiraTutorial(CenaBase):
    def __init__(self, tela, deck_jogador_global, proxima_depois="mapa"):
        ...
        try:
            ...
            self.fundo.fill((30, 20, 20)) # Tom levemente avermelhado/quente
        ...
        # os dialogos de quando encontrar a fogueira
        self.textos_intro = [
            "Você encontra um pequeno grupo de cangaceiros...",
            "Estão rodeando uma fogueira com os estômagos colados nas costas.",
            "Eles olham pro seu grupo de criaturas...",
            "Um deles fala: Venha cá, não se acanhe, esses seus bichos tão batendo os queixo de frio",
            "Outro completa: Aconchegue eles perto da fogueira. Isso vai melhorar a vida de um deles.",
            "Tu percebe que um deles lambe os beiço limpando a saliva que escorria pela boca."
        ]
        
        # os dialogos finais (depois da gente queimar as cartas)
        self.textos_final = [
        ...
        self.dialogo_atual = self.textos_intro[self.indice_dialogo]
        
        # diferentes estados: "dialogo_intro", "escolha_carta", "dialogo_final"
        self.estado = "dialogo_intro"
        
        largura, altura = self.tela.get_size()
        
        # O único slot no meio da tela que representará a fogueira após a inserção dos assets bonitinhos (se der tempo)
            ...
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                ...
                if self.estado == "dialogo_intro":
                    ...
                    if self.indice_destaque is not None:
                        ...
                        # o efeito que o evento dará (pode alterar isso e deixar random igual no inscryption, mas assim acho que ja ta bom)
                    ...
                    if self.indice_dialogo < len(self.textos_final):
                        ...
                        # devolve a carta upada pro deck antes de sair
    ...
    def desenhar(self):
        ...
        # desenha o slot da fogueira só quando for a hora de escolher ou no final
        if self.estado in ["escolha_carta", "dialogo_final"]:
            cor_borda = (255, 69, 0) # laranjinha
        ...
        # a box do dialogo, não fizemos um arquivo especifico para ela, entao evita alterar ou vai ficar distoando do resto do codigo (a menos que tu mude em todos)
