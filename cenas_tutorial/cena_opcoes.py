...
class CenaOpcoes(CenaBase):
    def __init__(self, tela, volume_musica=0.6, volume_efeitos=1.0):
        ...
        self.rect_musica_minus = pygame.Rect(largura // 2 - 200, 180, 80, 80)
        self.rect_musica_plus = pygame.Rect(largura // 2 + 120, 180, 80, 80)
        self.rect_efeitos_minus = pygame.Rect(largura // 2 - 200, 270, 80, 80)
        self.rect_efeitos_plus = pygame.Rect(largura // 2 + 120, 270, 80, 80)
    ...
    def desenhar(self):
        ...
        label_musica = self.fonte_texto.render("Música", True, (255, 255, 255))
        self.tela.blit(label_musica, (self.rect_musica_minus.left, self.rect_musica_minus.top - 30))
        label_efeitos = self.fonte_texto.render("Efeitos", True, (255, 255, 255))
        self.tela.blit(label_efeitos, (self.rect_efeitos_minus.left, self.rect_efeitos_minus.top - 30))

        valor_musica = self.fonte_texto.render(f"{int(self.volume_musica*100)}%", True, (218,165,32))
        self.tela.blit(valor_musica, (self.rect_musica_minus.right + 20, self.rect_musica_minus.centery - valor_musica.get_height() // 2))
        valor_efeitos = self.fonte_texto.render(f"{int(self.volume_efeitos*100)}%", True, (218,165,32))
        self.tela.blit(valor_efeitos, (self.rect_efeitos_minus.right + 20, self.rect_efeitos_minus.centery - valor_efeitos.get_height() // 2))
