...
class CenaCombateTutorial(CenaBase):
    def __init__(self, tela, deck_jogador, dados_da_fase, imagens_versos, imagens_cartas, imagens_ui):
        super().__init__(tela) 
        ...
        # Sistema de Diálogo de Derrota
        self.dialogos_pendentes = []
        self.dialogo_atual = ""
        self.derrotado_tutorial = False
        ...
        for comando in acoes:
            if comando["acao"] == "jogar_carta":
                ...
                if isinstance(c, dict):
                    nome_inimigo = c["nome"]
                    imagem_inimigo = c.get("imagem") or self.imagens_cartas.get(nome_inimigo)
    ...
    def atualizar(self, dt):
        ...
        if self.turno_atual == "resolvendo_combate":
            if self.fase_resolucao == "aliados":
                ...
                if self.idx_atacante_atual >= 4:
                    if self.peso_balanca <= -8:
                        self.derrotado_tutorial = True
                        self.resultado = "derrota"
                        self.adicionar_dialogos([
                            "Perdesse",
                            "Ver essa judiação como uma forma de aprendizado é uma leiguice",
                            "...",
                            "Bem, não é o fim",
                            "Ainda...",
                            "*Coloca os copos de shot na mesa*",
                            "Deixa eu te explicar uma coisa ou duas...",
                            "Esse foi um dos dois erros que tu podia cometer",
                            "Se você perde, você vira",
                            "Se acabar os shots...",
                            "*o jogador toma o shot*",
                            "Agora que tu sabe o básico",
                            "Vamo começar a história de verdade"
                        ])
                        return
